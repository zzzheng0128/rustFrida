...
当前兼容性实验只认 Pixel 6：`DEVICE_SERIAL=18201FDF6002GR`（Android 15，内核
`6.1.99-android14-11-gd6f926cfde54-ab12786694`）。Pixel 5 的 `4.19` 设备属于另一
组实验，不能与本机的 KPM、HWBP 或崩溃日志混用。运行器会在每轮结果中保存 serial、
model 和 kernel，便于复核。
...
```bash
# RustFrida 全部通道
DEVICE_SERIAL=18201FDF6002GR RUN_SECS=60 BUILD_RF=0 BUILD_APP=0 \
...
| `12` | `mkpm-redirect` | `eredirect <uid> addexact ...` | 同一 App 内关闭时 `open=-2`，开启后读到 `mkpm-redirect-target` |
| `13` | `mkpm-time` | `boot uid` + syscall 113 | App 的 `CLOCK_BOOTTIME` 与 root shell 相差约 600 秒 |
| `14` | `mkpm-inode` | `emaps <uid> addino ... 1` | 同一 App 内 maps inode 从原值变为 1 |
...
| `12 mkpm-redirect` | `eredirect <uid> addexact ...` + `hook` | App UID 的路径替换；runner 用 App 内前后读值判定 |
| `13 mkpm-time` | `boot uid <uid>` + `boot time 600 0` | `CLOCK_BOOTTIME` 与 shell 相差约 600 秒 |
| `14 mkpm-inode` | `emaps <uid> addino ...` + `hook` | 同一 App 内关闭/开启前后的 maps inode 差异 |
...
- **hide maps**：打开/关闭 maps 过滤后，App 的 maps 观察结果应变化，关闭后恢复；
  shell/root 的视角只作基线；
- **redirect（KPM 控制名 `eredirect`）**：只匹配指定 UID 和精确测试路径。runner 在同一 App 进程内先确认
  marker 不存在（`open=-2`），再开启规则并确认读到固定目标内容
  `mkpm-redirect-target`；前后对照都满足才算范围匹配正确，不依赖 shell 进程视角；
  `SUMMARY.txt` 会写入 `redirect_compare_scope=app-only`，表示判定只使用 App 自己的前后两次读取。
- **boot_time**：App 的 `CLOCK_BOOTTIME` 与 root shell `/proc/uptime` 的绝对差约
  600 秒（runner 允许 590--610 秒）；系统全局时钟不应改变；
- **inode/emaps**：规则关闭时记录测试 VMA 原 inode，开启 `addino ... 1` 后同一 App
  再读应为 1；`关闭=<原值>，开启=1` 才算按 UID 生效；
