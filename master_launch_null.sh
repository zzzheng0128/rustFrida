#!/system/bin/sh
# master_launch_null.sh — 方向 C 对照实验
# 与 v4 唯一的区别: 用 null_agent.js (空 agent, 无 hook)
# 目的: 验证纯 rustfrida spawn 能否让抖音稳定运行 30 分钟

DURATION="${1:-1800}"
PKG="com.ss.android.ugc.aweme"
LOG_DIR="/data/local/tmp/stress_null_$(date +%Y%m%d_%H%M%S)"
...
# ── 2. 启动 rustfrida spawn, 用 null_agent.js ──
log "启动 rustfrida spawn (null_agent): $PKG"
tail -f -n 0 "$KEEPALIVE" | /data/local/tmp/rustfrida --spawn "$PKG" --load-script /data/local/tmp/null_agent.js > "$LOG_DIR/rustfrida.log" 2>&1 &
RF_PID=$!
