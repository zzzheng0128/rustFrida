...
function testTarget(name, addr) {
    ...
    for (var j = 0; j < windows.length; j++) {
        if (baseline[windows[j]]) { anyBaseline = true; break; }
    }
    if (!anyBaseline) {
    ...
    function probe() {
        ...
        return { allOk: wxAllOk, results: wxResults, hits: wxHitCount };
    }

    var probe1 = probe();
    crclog("PROBE1",
           name + " hits=" + probe1.hits + " crc_match=" + probe1.allOk +
    ...
    for (var m = 0; m < windows.length; m++) {
        var bw = windows[m];
        if (baseline[bw]) stats.perTarget[name].baseline[bw] = baseline[bw].crc;
        if (probe1.results[bw]) stats.perTarget[name].wxResults[bw] = probe1.results[bw].crc;
    }
...
function installAll() {
    ...
    crclog("CFG", "candidates=" + candidates.length + " (windows 4/8/16/32/64/128)");
    ...
    for (var k = 0; k < keys.length; k++) {
        ...
        for (var w = 0; w < ws.length; w++) {
            var ww = ws[w];
            if (t.baseline[ww] !== undefined) {
                wxTotal++;
                if (t.wxResults[ww] === t.baseline[ww]) wxOk++;
                normalTotal++;
                if (t.normalResults[ww] !== undefined &&
                    t.normalResults[ww] !== t.baseline[ww]) normalDiff++;
            }
        }
        summary.push({
            name: key,
    ...
    console.log("[CRC] WXSHADOW_crc32_bypass=" + (allWxCrc ? "PASS (所有窗口 CRC 与原始一致)" : "FAIL (有窗口 CRC 变化)") +
                "  NORMAL_control=" + (allNormalCrc ? "PASS (NORMAL 必坏 CRC)" : "FAIL (NORMAL 没坏 CRC 说明检测链路也失效)") +
                "  命中旁证=" + (anyHits ? "有命中" : "无命中（看 dmesg exec/read fault）"));
}
